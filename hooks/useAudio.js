...
export function useAudio() {
  ...
  useEffect(() => {
    async function loadSounds() {
      try {
        const { sound: tap } = await Audio.Sound.createAsync(require('../assets/sounds/tap.mp3'));
        tapSound.current = tap;

        const { sound: death } = await Audio.Sound.createAsync(require('../assets/sounds/death.mp3'));
